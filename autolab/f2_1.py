...
def pass_check(password):
    ...
    flag,count,counter,value,badval,splval=False
    p=list(password)
    #print(p)
    spl=["@","#","$"]
    notreq='!%^&*()_-+=/*~`<>?,./:";|'
    ...
    if ((6<=len(p)<=16) ) :
        ...
        for i in p:
            if (i.islower()):
                ...
                badval+=True           
        print(flag)
    if (flag==True and value==True and counter==True and count==True and splval==True and badval==False):
        return True                    
    else:
        return False
...
Then I take all the odd indexed numbers from second sub-list which is : 0 (since it's index is 1)
